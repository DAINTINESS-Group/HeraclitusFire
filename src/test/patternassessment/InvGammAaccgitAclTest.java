...
class InvGammAaccgitAclTest {
	...
	static void setUpBeforeClass() throws Exception {
		...
		invGammaAssessment = new InverseGammaAssessment(inputTupleCollection, "accgit__acl", "resources/test/InvGamma", "resources/test/GlobalLog.txt", ALPHA);
	...
	final void testComputeContingencyTable() {
		...
		assertEquals(contingencyTable[0][0], 5);
	...
	final void testDecideIfPatternHolds() {
	  ...
	  final void testAssessPattern() { 
		  File fileProduced = new File("resources/test/InvGamma/Accgit__acl_InverseGamma.txt"); 
